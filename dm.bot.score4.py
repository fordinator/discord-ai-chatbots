...
class SetTurnLimitView(discord.ui.View):
    ...
    async def set_limit(self, interaction: discord.Interaction, limit: Optional[int]):
        ...
        if limit:
            ...
            message = "✅ **No Turn Limit Set.** Play as long as you wish, but your score will be penalized for sessions longer than 10 turns."
...
class RPGActionView(ui.View):
    def __init__(self, session: RPGSession, trivial_actions: List[str]):
        ...
        # NEW: Accumulating Value (AV) display
        av_button = ui.Button(
            label=f"AV: {session.accumulating_value:+}",
            style=discord.ButtonStyle.blurple,
            custom_id="rpg_av_display",
            disabled=True,
            row=2 # Row index 2
        )
        self.add_item(av_button)

        # Trivial action buttons (up to 2 more rows)
        for i, action in enumerate(trivial_actions[:2]): # Limit to 2 to fit with AV button
            button = ui.Button(
                label=action[:80],
                style=discord.ButtonStyle.primary,
                custom_id=f"rpg_action_{i}",
                row=3 + i # Rows 3 and 4
            )
...
async def narrate_rpg_turn(channel: discord.TextChannel, session: RPGSession, llm_payload: list, client: AsyncOpenAI, message_to_reply: discord.Message, roll_details_text: str = "", event_notification: Optional[str] = None):
    try:
        ...
        trivial_actions_prompt = "Based on the last scene description, suggest two distinct, simple, one-sentence actions a player could take. These actions should be trivial and not require a skill check. Please ensure that you use fewer than 8 words for these options. Respond *only* with a valid JSON object containing a single key 'actions' with a list of 1-2 strings. Example: {\"actions\": [\"Look under the bed.\", \"Open the window.\"]}"
...
class RollDiceView(discord.ui.View):
    ...
    async def roll_button_callback(self, interaction: discord.Interaction):
        ...
        # --- MODIFIED: Apply Accumulating Value (AV) to the roll ---
        av_bonus = self.session.accumulating_value
        final_result = (pos - neg) + stat_bonus + fv_bonus + av_bonus
        ...
        # --- MODIFIED: Update AV based on outcome ---
        if outcome_desc == "Amazing success":
            self.session.accumulating_value += 1
        elif outcome_desc == "Terrible failure":
            self.session.accumulating_value -= 1
        ...
        roll_summary = (f"**Action:** *{self.session.pending_action_text}*\n"
                        ...
                        # --- MODIFIED: Display AV in roll calculation ---
                        f"**Roll:** `[{pos} - {neg}]` (Roll) `+ {stat_bonus}` (Stat) `+ {fv_bonus}` (FV) `+ {av_bonus}` (AV) `=` **{final_result}**\n"
...
# --- REWORKED: This function now only calculates the score and returns the results ---
async def _calculate_final_score(session: RPGSession, client: AsyncOpenAI, model: str) -> (int, discord.Embed):
    base_score = 0
    base_score_reason = ""
    ...
    # --- MODIFIED: Turn baseline is now 10 ---
    turn_diff = session.current_turn - 10
    multiplier = 0
    if turn_diff > 0: 
    ...
    embed.add_field(name="✨ Fancy Value Bonus", value=f"`{fv_bonus:+}` _({session.high_fv_actions_count} actions with FV ≥ +4)_", inline=False)
    ...
    embed.add_field(name="⏳ Turn-Based Multiplier", value=f"`{multiplier:.0%}` _({session.current_turn} turns vs. a baseline of 10)_", inline=False)
    embed.add_field(name="⭐ Final Score", value=f"**`{final_score} / 100`**", inline=False)
    
    return final_score, embed

# --- REWORKED: This function now calculates the score BEFORE generating the conclusion ---
async def _conclude_rpg_session(channel: discord.TextChannel, session: RPGSession, conclusion_reason: str):
    await channel.send(f"**The adventure concludes...**\n`Calculating final score and preparing epilogue...`")

    provider, model = curr_model.split("/", 1)
    client = AsyncOpenAI(base_url=config["providers"][provider]["base_url"], api_key=config["providers"][provider].get("api_key", "sk-no-key-required"))

    # Step 1: Calculate the final score and get the embed for later.
    final_score, score_embed = await _calculate_final_score(session, client, model)
    score_embed.description = f"**Character:** {session.user_mention}\n**Conclusion:** {conclusion_reason}"

    # Step 2: Create a new prompt for the score-based conclusion.
    conclusion_prompt = (
        f"The adventure is now over. The reason for the conclusion is: '{conclusion_reason}'. "
        f"The player's final calculated score for this adventure is **{final_score} out of 100.**\n\n"
        "Your task is to write a concluding narrative epilogue that reflects this score. Do not mention the score number itself. "
        "A high score (75-100) should result in a heroic, positive, and triumphant conclusion. "
        "A low score (0-25) should result in a tragic, bleak, or pathetic conclusion. "
        "A medium score (26-74) should be mixed, perhaps a bittersweet victory or a costly survival. "
        "Based on the story so far and the final score, describe the character's performance, the immediate consequences of their final actions, the long-term consequences of their journey, and the final state of the world."
    )
    ...
    try:
        ...
        # Step 4: Send the score embed and clean up.
...
async def on_message(new_msg: discord.Message):
    ...
    if session.state == RPGState.AWAITING_DESCRIPTION:
        ...
        # --- MODIFIED: Simplified FV Calculation ---
        fv_bonus = 0
        fv_details_parts = []
        ...
        # --- MODIFIED: Changed FV bonus threshold to +4 ---
        if fv_bonus >= 4:
