...
# --- NEW: HELPER FUNCTION FOR DISTANCE ---
def _update_distance(current_distance: str, steps: int) -> str:
    """
    Updates a participant's distance by a number of steps.
    - Positive steps move closer, negative steps move further.
    - Cannot move past 'extreme' or into 'grapple' via jog/sprint.
    """
    try:
        current_index = DISTANCE_LEVELS.index(current_distance)
    except ValueError:
        return current_distance # Should not happen

    # Sprinting (2 steps) or Jogging (1 step) moves closer
    new_index = current_index - steps
    
    # Clamp the index. Player movement cannot enter 'grapple' (index 4).
    # It stops at 'close' (index 3).
    new_index = max(0, min(3, new_index)) 

    return DISTANCE_LEVELS[new_index]

# --- START: NEW COMBAT UI VIEWS ---
...
class PlayerCombatTurnView(discord.ui.View):
    def __init__(self, session: RPGSession):
        super().__init__(timeout=None) # Combat turn doesn't time out
    ...
    async def on_target_selected(self, interaction: discord.Interaction):
        ...
        # 1. Ranged Attack phase
        # Player can't use ranged attacks if target is already close or in grapple
        if target.distance not in ["close", "grapple"]:
            ...
            if ranged_attack_view.choice is True:
                ...
                # Here we would end the player's turn and pass to NPC
                return
            # If choice is False or view times out, we proceed to movement
        
        # 2. Movement phase
        movement_view = MovementView(self.session.user_id)
        # Use followup if we already responded ephemerally, otherwise use the original interaction response
        send_method = interaction.followup.send if interaction.response.is_done() else interaction.response.send_message
        await send_method("How do you want to move?", view=movement_view, ephemeral=True)
        await movement_view.wait()
        
        if not movement_view.choice: # Handle timeout
            await interaction.followup.send("You stood still, pondering your next move. Your turn ends.", ephemeral=True)
            return

        # 3. Process movement and evaluate result
        ...
        # If player sprinted, their turn ends.
        if movement_view.choice == "Sprint":
            ...
            # End turn and pass to NPC
            return

        # If player jogged and is now in close range, offer a melee attack.
        if movement_view.choice == "Jog" and target.distance == "close":
            melee_attack_view = ConfirmAttackView(self.session.user_id, "Melee")
            await interaction.followup.send(f"You are now in close range of **{target.name}**. Do you want to make a Melee Attack?", view=melee_attack_view, ephemeral=True)
            await melee_attack_view.wait()

            if melee_attack_view.choice is True:
                # Placeholder for melee attack logic
                await interaction.followup.send(f"You swing at **{target.name}**! (Attack logic to be implemented). Your turn ends.", ephemeral=True)
            else:
                await interaction.followup.send("You hold your ground, ready for their next move. Your turn ends.", ephemeral=True)
            # End turn and pass to NPC
            return

        # If player jogged but isn't in close range, their turn just ends.
        await interaction.followup.send("Your turn ends.", ephemeral=True)
        # End turn and pass to NPC


# --- END: NEW COMBAT UI VIEWS ---
...
async def _initialize_combat(channel: discord.TextChannel, session: RPGSession, client: AsyncOpenAI, model: str):
    ...
    # --- MODIFIED: PRESENT PLAYER TURN VIEW ---
    embed = discord.Embed(
    ...
    # Present the main combat view to the player to start their turn.
