...
class RPGSession:
    ...
    inventory: List[GearItem] = field(default_factory=list) # ADD THIS LINE
    combat_module_enabled: bool = False # ADD THIS LINE
...
async def _start_gear_generation(channel: discord.TextChannel, session: RPGSession, client: AsyncOpenAI, model: str):
    ...
    # --- Apply Bonuses and Continue Adventure ---
    ...
    # --- Continue with the rest of the adventure setup ---
    ...
    plot_generation_prompt = (
        # ... (This is the same plot generation prompt from the original function)
    ...
    #... etc, the rest of the original setup logic follows
    #...
    # The following is the rest of the logic to start the game, copied from the previous version.
    ...
    # Update system prompt with final combat stats INCLUDING gear bonuses
    if session.combat_stats:
        ...
        # Find and update the system message with stats
...
lass ConfirmItemView(discord.ui.View):
...
async def _continue_adventure_setup(channel: discord.TextChannel, session: RPGSession, description_provided: bool):
    ...
    # Hand off to the new gear generation system
