...
class RPGSession:
    ...
    # NEW FIELDS FOR SCORING AND TURN TRACKING
...
# NEW VIEW for setting the turn limit
...
async def _start_gear_generation(channel: discord.TextChannel, session: RPGSession, client: AsyncOpenAI, model: str):
    ...
    await gear_message.edit(content="**Gear confirmed and bonuses applied!**", embed=inventory_embed, view=None)

    await channel.send(content="*The threads of fate are weaving a secret destiny for you...*", silent=True)

    character_desc = next((p['content'] for p in session.story if p['role'] == 'system' and 'Player description' in p['content']), "The player has not provided a character description.")
    ...
    plot_generation_prompt = (
        ...
        f"The player's character is described as follows: '{character_desc}'\n"
    ...
    if plot_json:
        ...
        # --- STEP 0: GENERATE AND ADD OVERALL GOAL ---
        goal_prompt_text = (
            ...
            "If the character description is generic, invent a compelling goal. This goal will be shown to the player. "
        ...
        try:
            ...
            session.plot_outline['OVERALL_GOAL'] = overall_goal # Add to secret plot
        except Exception as e:
...
class RPGActionView(ui.View):
    ...
    async def on_trivial_action_click(self, interaction: discord.Interaction):
        ...
        # --- MODIFIED: Increment turn counter on player action ---
...
async def narrate_rpg_turn(channel: discord.TextChannel, session: RPGSession, llm_payload: list, client: AsyncOpenAI, message_to_reply: discord.Message, roll_details_text: str = "", event_notification: Optional[str] = None):
    try:
        ...
        # MODIFICATION: Check if game has concluded to avoid replying to a deleted message
        try:
            response_msg = await message_to_reply.reply(embed=embed, silent=True)
        except discord.NotFound: # The original message was deleted (likely by conclusion logic)
        ...
        # If player hit their turn limit, conclude the game immediately.
        # --- MODIFICATION TO FOLLOW STEP 7 ---
        # This auto-conclusion is now removed per Step 7's clarification.
        # if session.turn_limit and session.current_turn >= session.turn_limit:
        #     await response_msg.edit(content="**You have reached your turn limit!**", embed=None, view=None)
        #     reason = f"The player reached their self-imposed turn limit of {session.turn_limit}."
        #     await _conclude_rpg_session(channel, session, reason)
        #     return
        ...
        # --- STEP 2: Display Turn count in narration ---
...
class RollDiceView(discord.ui.View):
    ...
    async def roll_button_callback(self, interaction: discord.Interaction):
        ...
        # --- MODIFICATION: Store margin for final scoring ---
        ...
        if outcome_desc == "Terrible failure":
            ...
            if "YES" in death_result:
                ...
                # --- MODIFICATION: Set death flag and pass specific reason ---
...
# --- NEW: SCORING CALCULATION AND DISPLAY ---
async def _calculate_and_display_final_score(channel: discord.TextChannel, session: RPGSession, client: AsyncOpenAI, model: str):
    await channel.send("`Analyzing adventure records and calculating final score...`")
    
    # Steps 3, 4, 5: Determine Base Score
    base_score = 0
    base_score_reason = ""
    ...
    if session.player_is_dead:
        ...
        # Check if antagonist is dead
        antagonist = session.plot_outline.get('PRIMARY_ANTAGONIST', 'the antagonist')
        ...
        # If antagonist isn't dead, get LLM evaluation of goal
        if not session.antagonist_is_dead:
            ...
            try:
                ...
                base_score = 50 # Default score on error
    ...
    multiplier = 0
    if turn_diff > 0: # Penalty for going over 7 turns
        multiplier = -0.10 * turn_diff
    elif turn_diff < 0: # Bonus for being under 7 turns
        multiplier = -0.10 * turn_diff # a negative * a negative is a positive
        
    final_score *= (1 + multiplier)
    final_score = int(max(0, min(100, final_score)))

    # Display results
    embed = discord.Embed(title="Adventure Complete: Final Score", color=EMBED_COLOR_COMPLETE)
    embed.description = f"**Character:** {session.user_mention}\n**Final Outcome:** {base_score_reason}"
...
async def _conclude_rpg_session(channel: discord.TextChannel, session: RPGSession, conclusion_reason: str):
    ...
    # Use the channel's last message as a reply target, or send a new one.
    try:
    ...
    try:
        ...
        # --- NEW: Trigger final scoring after conclusion ---
        await _calculate_and_display_final_score(channel, session, client, model)
...
async def on_message(new_msg: discord.Message):
    ...
    if session.state == RPGState.AWAITING_DESCRIPTION:
        ...
        # --- MODIFICATION: Increment turn count on player action ---
        # This is now done right before narration to capture all action types
        ...
        # --- MODIFICATION: Store high FV action count for scoring ---
        if fv_bonus >= 6:
        ...
        if ev < -6 or ev > 6:
            session.current_turn += 1 # Increment turn for this action
        ...
        if -2 <= cv <= 2:
            ...
            # Note: Turn is incremented inside the RollDiceView for this path
