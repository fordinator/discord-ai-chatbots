...
trivia_sessions: dict[tuple[int, int], TriviaSession] = {}
...
async def reset(interaction: discord.Interaction):
    ...
    system_prompt = config["system_prompt"]
    await interaction.response.send_message(f"**System prompt reset.**")
...
async def models(interaction: discord.Interaction):
    ...
    try:
        ...
        available_models = [model.id async for model in models_list]
        if not available_models:
            reply = "No models found."
        else:
            reply = f"**Available {provider} models:**\n" + "\n".join(available_models)
    except Exception as e:
        logging.exception(f"Error fetching models from {provider}")
        reply = f"Error fetching models: {e}"
...
async def on_message(new_msg: discord.Message) -> None:
    ...
    if session_key in trivia_sessions:
        ...
        answer_embed = discord.Embed(
            title = f"Answer to Question {session.current_index + 1}",
        ...
        if session.current_index >= len(session.questions):
            end_embed = discord.Embed(
                description="✅ Trivia complete! Thanks for playing.",
            ...
            question_embed.set_footer(text=f"Question {session.current_index + 1} of 3 | Reply to this message to answer.")
            await new_msg.channel.send(embed=question_embed)
            
        return
        
    """ END OF CODE FOR TRIVIA """
