...
class RPGActionView(ui.View):
    def __init__(self, session: RPGSession, trivial_actions: List[str]):
        ...
        for i, action in enumerate(trivial_actions[:2]):
            button = ui.Button(
                label=action[:80],
...
class RollDiceView(discord.ui.View):
    ...
    async def roll_button_callback(self, interaction: discord.Interaction):
        ...
        outcome_prompt = (
            ...
            "Narrate the tangible CONSEQUENCES of this result. The outcome should be powerful and beneficial, but grounded in the established reality of the world. "
            "Avoid cosmic, spiritual, or reality-altering events unless they are a direct and plausible extension of the player's specific action. "
            "Describe what HAPPENS NOW, ensuring the situation has fundamentally changed. "
            "**Your response must NOT offer the player a list of choices or suggested actions.** You must conclude your narrative with only the open-ended question, 'What do you do now?'"
        )
...
async def _calculate_final_score(session: RPGSession, client: AsyncOpenAI, model: str) -> (int, discord.Embed):
    ...
    # --- FIXED: Corrected the final score calculation logic ---
    ...
    final_score = int(subtotal * (1 + multiplier))
