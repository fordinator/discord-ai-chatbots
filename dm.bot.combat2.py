...
# --- NEW DATA CLASSES FOR COMBAT ---
@dataclass
class CombatParticipant:
    """Represents a single participant in combat, besides the player."""
    name: str
    description: str
...
@dataclass
class CombatState:
    """Holds the overall state of a combat encounter."""
    participants: List[CombatParticipant] = field(default_factory=list)
    bystanders: List[str] = field(default_factory=list) # For those not in the initial fight
    turn: int = 1
...
class RPGState(Enum):
    ...
    SETUP = auto()
    COMBAT = auto() # Add this new state
...
class RPGSession:
    ...
    # --- NEW: Add combat_state to the session ---
...
# This is a new helper function, place it near your other LLM helper functions

async def _check_if_combat_is_needed(client: AsyncOpenAI, model: str, session: RPGSession, action_text: str) -> bool:
    """Uses the LLM to determine if the player's action should trigger combat."""
    prompt = (
...
def _apply_gear_bonuses(session: RPGSession):
    """Applies all inventory bonuses to the character's combat stats."""
...
async def _get_llm_trinket_effect(client: AsyncOpenAI, model: str, description: str) -> str:
    """Generates a 'strange effect' for a special trinket."""
...
# --- START: NEW COMBAT INITIALIZATION FUNCTIONS ---

def _generate_npc_stats() -> Dict[str, int]:
    """Generates a simple dictionary of stats for an NPC with values from -1 to 3."""
...
async def _initialize_combat(channel: discord.TextChannel, session: RPGSession, client: AsyncOpenAI, model: str):
    """
    Parses the story to create combat participants, generates their stats,
    and sets up the initial combat state.
    """
    ...
    # 1. Use LLM to identify combatants
    participant_prompt = (
    ...
    if boss_data := participant_json.get("boss"):
        ...
        participant = CombatParticipant(
            ...
            distance=random.choice(["extreme", "distant", "medium", "close", "grapple"]),
            is_boss=True,
            stats=boss_stats,
    ...
    if mook_data_list := participant_json.get("mooks"):
        for mook_data in mook_data_list[:3]: # Ensure max 3 mooks
            mook_stats = _generate_npc_stats()
            combat_stats = _calculate_combat_stats(mook_stats)
            # Mooks do NOT get the x3 HP/MP multiplier
    ...
    # 3. Present the initial combat situation to the player
    ...
    embed = discord.Embed(title="Combat Begins!", color=EMBED_COLOR_RPG)
    for p in session.combat_state.participants:
        ...
        embed.add_field(
            name=f"[{role}] {p.name} ({p.distance.title()})",
    ...
    # NOTE: The full combat loop would start here. For now, we stop after initialization.
    # To prevent the bot from getting stuck, we'll revert the state for now.
    session.state = RPGState.IDLE 

# --- END: NEW COMBAT INITIALIZATION FUNCTIONS ---


async def _begin_adventure(channel: discord.TextChannel, session: RPGSession, client: AsyncOpenAI, model: str):
    """Initializes stats, generates the plot, and narrates the opening scene."""
    
    # Set the state to IDLE to activate the main game loop handler.
    session.state = RPGState.IDLE

    # Initialize combat stats. If gear was skipped, these will be the final stats.
    session.combat_stats = _calculate_combat_stats(session.stats)
    # Apply gear bonuses. This does nothing if inventory is empty.
    ...
    # Update system prompt with final combat stats INCLUDING any gear bonuses
...
async def _start_gear_generation(channel: discord.TextChannel, session: RPGSession, client: AsyncOpenAI, model: str):
    """The main controller for the entire gear generation process."""
    
    while True: # This loop allows for the "Discard and Start Over" functionality
        ...
        for item_type, details in GEAR_CATALOG.items():
            ...
            await confirm_view.wait() # Wait for user to click "Yes" or "No"

            # The interaction from the button click is stored in the view now.
            interaction = confirm_view.interaction
            
            if confirm_view.choice:
                # --- START: REPLACEMENT FOR wait_for ---
                description_text = None
                modal = GearDescriptionModal(item_type)
                
                # Show the modal to the user
                await interaction.response.send_modal(modal)
                await modal.wait() # Wait for the user to submit the modal

                if modal.description:
                    description_text = modal.description
                else:
                    # Modal timed out or was closed
                    await gear_message.edit(content="Timed out waiting for a description. Moving on...", embed=None, view=None)
                    continue
                # --- END: REPLACEMENT FOR wait_for ---

                # Handle special cases for Trinket and Special Move
                ...
                if item_type == "Special Move":
                    ...
                    # We edit the original message that holds the whole gear process
                ...
                # User clicked "No", the loop will continue to the next item
                pass
        ...
        if finalize_view.choice:
            # --- EDIT ---
            # Edit the final message and then simply return.
            await gear_message.edit(content="**Gear confirmed!**", embed=inventory_embed, view=None)
            break # Player chose to Keep, exit the while loop
        # If discard, the loop continues and everything starts over

    # --- REMOVE EVERYTHING FROM HERE TO THE END OF THE FUNCTION ---
    # The rest of the function (starting the adventure) has been moved to _begin_adventure
    return

class GearDescriptionModal(discord.ui.Modal, title="Describe Your Item"):
    """A modal for collecting an item's name and description."""
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Defer the interaction so it doesn't time out while we continue the gear logic.
        await interaction.response.defer()
        self.stop() # Stops the modal's wait()
...
class ConfirmItemView(discord.ui.View):
    def __init__(self, user_id: int):
        ...
        self.choice = None
        self.interaction: Optional[discord.Interaction] = None # Add this line
...
def _calculate_combat_stats(stats: Dict[str, int]) -> 'CombatStats':
    """
    Calculates the derived combat statistics from a character's base stats
    based on the predefined formulas.
    """
    ...
    # Calculate base values, rounding down as required.
    ...
    # Create and return the CombatStats object, applying minimums and multipliers.
    return CombatStats(
...
async def _continue_adventure_setup(channel: discord.TextChannel, session: RPGSession, description_provided: bool):
    """Asks about combat module, then hands off to the gear generation system."""
    ...
    # Only run gear generation if the combat module was enabled by the user.
    if session.combat_module_enabled:
        await _start_gear_generation(channel, session, client, model)

    # Now, unconditionally begin the adventure.
    # The _begin_adventure function will handle both cases correctly.
...
async def on_message(new_msg: discord.Message):
    ...
    # --- NEW LOGIC ---
    # If there is no session OR the session is in the SETUP state,
    # the global handler should do nothing. This allows `wait_for` to work uninterrupted.
    if not session or session.state == RPGState.SETUP:
        return
    ...
    if session.state == RPGState.AWAITING_DESCRIPTION:
        # This message was the original "Please reply with your description" prompt. Delete it.
        ...
        # The character description phase is over. 
        # Set the state to SETUP to signal that the multi-step setup process is starting.
        # This will prevent on_message from interfering with subsequent setup prompts.
        session.state = RPGState.SETUP 
        
        # Now, begin the next part of the setup (Combat Module, Gear, etc.)
    ...
    # STATE 2: Player is taking an action during the game
        ...
        # --- MODIFIED: EVALUATE IF COMBAT IS NEEDED ---
        ...
        if combat_needed and session.combat_module_enabled:
            # If combat is needed and the module is on, start combat.
            await _initialize_combat(new_msg.channel, session, client, model)
            return
        
        # --- IF NO COMBAT, PROCEED WITH NARRATIVE ACTION ---
        # This block now handles three cases:
        # 1. Combat was not needed.
        # 2. Combat was needed, but the module is disabled.
        # 3. An Erratic Event bypasses the normal flow.

        additional_context = []
        if combat_needed and not session.combat_module_enabled:
            # Add context for the LLM to guide its narrative when combat is bypassed.
        ...
        # --- The rest of the logic is the existing narrative flow (FV, EV, CV, etc.) ---
        fv_bonus = 0
        fv_details_parts = []
        ...
        cv_json = await get_llm_json_response(client, session, cv_payload) # Note: get_llm_json_response forces vanilla personality
        ...
        if -2 <= cv <= 2:
            ...
            # This logic for determining stat and DV for a roll remains the same.
